...
public class TPA extends BukkitCommand {
	...
	public TPA(String name, Teleportation_Main instance) {
		super(name);
		this.description = "Request teleportation to a player";
				...
				if(!teleport.cooldown.contains(player.getName())) {
					if(args.length > 0) {
						...
						if(Bukkit.getPlayer(targetName) != null){
							...
							teleport.tpRequests.put(targetName, new tpRequest(senderName, false));
							Player targetPlayer = Bukkit.getPlayer(targetName);
				
							targetPlayer.sendMessage(ChatColor.GREEN + senderName + " has requested to teleport to you!\n/tpaccept - Accept the request\n/tpdeny - Deny the request\nThe request will time out in 30 seconds.");
						...
						player.sendMessage(ChatColor.RED + "Syntax Error: /tpa <player>");
					}
				} else {
